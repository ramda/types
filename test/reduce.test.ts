...
import { __, reduce, pipe, compose, toString, map } from '../es';
...
expectType<(list: readonly string[]) => number>(pipe(map(Number.parseInt), reduce(sum, 0)));
expectType<(list: readonly number[]) => string>(pipe(reduce(sum, 0), toString));
