...
export function assoc<K extends PropertyKey>(prop: K): {
  ...
  <T>(val: T): {
    ...
    <U>(obj: U): U extends Record<K, any> ? T extends U[K] ? U : Omit<U, K> & Record<K, T> : Omit<U, K> & Record<K, T>;
  }

  // assoc(prop)(__, obj)
  <U>(__: Placeholder, obj: U): {
    // assoc(prop)(__, obj)(val), when obj has key prop, tests if val is typeof obj[prop] for best return type
    <T>(val: T): U extends Record<K, any> ? T extends U[K] ? U : Omit<U, K> & Record<K, T> : Omit<U, K> & Record<K, T>;
  }

  // assoc(prop)(val, obj) when obj has key prop, tests if val is typeof obj[prop] for best return type
  <T, U>(val: T, obj: U): U extends Record<K, any> ? T extends U[K] ? U : Omit<U, K> & Record<K, T> : Omit<U, K> & Record<K, T>;
};
